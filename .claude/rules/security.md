...
| Field | Value |
|-------|-------|
| **Model** | None — this repository contains no application code (no backend service, no API layer, no frontend app). There is nothing to authenticate against. |
| **Mechanism** | N/A — no request-handling code exists anywhere in the repository. |
| **Access primitive** | N/A — no resources, ownership model, or ACL exist. |
| **Roles / scopes** | None defined — no role, scope, or permission enum exists anywhere in the repository. |

**No frontend exists in this repository either.** SEC-01 through SEC-10 below are written for when backend and/or frontend code is added to this repo; until then there is no endpoint, UI, or data flow for any of them to check.
