---
paths:
  - "backend/**" # No backend code/directory exists in this repository yet — this repo currently has no application code at all (no package.json, build file, or source tree of any language). Placeholder scoped to a future backend/ directory so this rule stays dormant (matches nothing) rather than firing repo-wide until real backend code and its language are known.
---
