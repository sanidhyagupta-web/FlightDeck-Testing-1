...
**No frontend exists in this repository yet.** There is no source tree, `package.json`, build config, or CI workflow here — nothing to derive a real test command from. The framework and code samples below are the generic template this skill scaffolds with; replace them with this repo's real stack and commands once a frontend is actually added.
...
No test runner is configured in this repository yet (no `package.json` exists to read a script from). Fill this in once a frontend and its tooling are added:
```bash
