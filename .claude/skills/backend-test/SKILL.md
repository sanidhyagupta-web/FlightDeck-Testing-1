...
**No backend exists in this repository yet.** There is no source tree, package manifest, build file, or CI config of any kind here — nothing to derive a real test command from. The strategy and code samples below are the generic template this skill scaffolds with; replace them with this repo's real stack and commands once a backend is actually added.
...
No build tool or test runner is configured in this repository yet (no `pom.xml`, `build.gradle`, `package.json`, or CI workflow exists to read a command from). Fill this in once a backend and its build tooling are added:
```bash
